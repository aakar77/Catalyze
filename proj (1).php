...
$sql1="Select uname from user u where u.uid=$result[projcreatorid]";
...
$interval = $deadline->diff($now);
                echo "<h1 class='page-header'>$result[projname]
                    <small>by $row1[uname] </small> <p>Posted on ".date_format(date_create($result['projpostingdatetime']),'jS \o\f F,Y')." </p>
...
?>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/5.0.0/normalize.min.css">
    ...
    echo $i;
    echo "<div id='com'>";
    echo "inside div".$i=$_POST['projid'];
    $sql9="Select * from comment where projid=$i";
    $fetch9=mysqli_query($con,$sql9);
...
function insertcomment()
{
    ...
    $.ajax({
                ...
                success: function(data)
                {
                    if(data.status==1)
                        alert(data);
                        $("#com").load("http://localhost/project/projsearch/proj.php #com");
                },
                error: function()
                {
                    alert("No");
